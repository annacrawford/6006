...
polarplot(dayspeed, dayaz, theta.zero = pi/2, theta.clw = TRUE, method = 1,
          ...
          tlabel.col = "black", main = "Ice island daily velocity (km/d)", sub = substring(fname1,1,5))

    #Delete existing files
    deleteFile(paste("polarplot_", substring(fname1,1,5), sep=""), c("png"))
    
    #Save polarplot to output file  
dev.copy(png, paste("polarplot_", substring(fname1,1,5), ".png", sep=""))
dev.off()
 }