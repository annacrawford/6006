### Script to take  beacon files to a common, standard csv format for further 
# processing in script 'beacon_processing.r'

# Started by Anna Crawford - Feb 16/2013
# Last updated by Anna Crawford - Feb 16/2013

#check line 125 - complete cases, not needed for Joubeh, but possibly for SailWx/Iridium
#check line 134 - change working directory to ProcessedData if an iridium file 
####################################################################################
...
#gmail/sbd
#setwd("C://RProj/beacons/beacon_data/RawData")


#joubeh/sailwx (went through Joubeh2csv or Sailwx2csv first)
setwd("/tank/HOME/acrawford/6006/ProcessedBeaconData")

#change to specific beacon using Joubeh
fname1 = "BUOY_4.csv"      #-# name file with ext
...
datetime = strftime(strDateTime, format="%Y-%m-%d %H:%M:%S", tz="UTC") #added 'format=' and tz="UTC"
datetime = as.POSIXct(datetime, tz="UTC")
gps_time = datetime

obs = seq(1,length(beacon$year))
beacon$beacon = as.character(beacon$beacon)
...
#not needed for Joubeh cases?
#beacon<-beacon[complete.cases(beacon[,1:15]),]


# get a filename for csv 
fout = fname1

#run if a gmail/iridium
setwd("C://Users/acrawford/Dropbox/RProj/beacons/beacon_data/ProcessedData")
